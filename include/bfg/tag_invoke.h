...
template <typename Tag, typename... Args>
auto tag_invoke_check(int) noexcept(noexcept(
...
struct tag_invoke_is_nothrow
{
	static constexpr bool value = std::integral_constant<
		bool,
		noexcept(tag_invoke_detail::tag_invoke_check<Tag, Args...>(0))>::value;
};
...
struct tag
{
	...
	{
		return ::bfg::tag_invoke(
			*static_cast<const Tag *>(this), ::std::forward<Args>(args)...);
	}
};
